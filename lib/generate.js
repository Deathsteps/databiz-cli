...
var { buildTableColumns, buildFilterFieldConfig, buildFilterForm, buildEditFormItems } = require('./schema2code')
...
function generateViews (cwd, templatesPath, model, schemaPath) {
  ...
  fs.writeFileSync(path.join(cwd, `src/views/${data['Model']}List/index.js`), code)
  // Filter.jsx
  templateFile = path.join(templatesPath, 'code/view/Filter.jsx.tmpl')
  code = formatCode(utils.generateCode(templateFile, data))
  fs.writeFileSync(path.join(cwd, `src/views/${data['Model']}List/Filter.jsx`), code)
  // DataTable.jsx
  templateFile = path.join(templatesPath, 'code/view/DataTable.jsx.tmpl')
  code = formatCode(utils.generateCode(templateFile, data))
  fs.writeFileSync(path.join(cwd, `src/views/${data['Model']}List/DataTable.jsx`), code)
  // Detail.jsx
  templateFile = path.join(templatesPath, 'code/view/Detail.jsx.tmpl')
  code = utils.generateCode(templateFile, data)
  fs.writeFileSync(path.join(cwd, `src/views/${data['Model']}/index.js`), code)
  // InfoForm.jsx
  templateFile = path.join(templatesPath, 'code/view/InfoForm.jsx.tmpl')
  code = formatCode(utils.generateCode(templateFile, data))
  fs.writeFileSync(path.join(cwd, `src/views/${data['Model']}/InfoForm.jsx`), code)
